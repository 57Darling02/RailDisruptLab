...
- 参考集 build run：`/home/wula/zyjiang/RailGraph2Gurobi/outputs/bench_build/2026-05-25_16-13-18`。
- 本报告由 `scripts/evaluate_ablation_similarity.py` 可重复生成；完整 JSON/CSV 输出位于 `outputs/gnn_steps_ablation/similarity_eval/latest/`。
- 评估不重跑训练、generate、build 或 solve，只读取既有 run 产物。
- `limit_speed == 0` 计为中断，`limit_speed > 0` 计为普通限速；晚点时长、限速持续时长、限速值等扰动程度指标均使用分箱 JSD。
- 当前旧 solve CSV 没有 `num_nodes` 和 `t_first_feas`，分支节点数与首个可行解时间不做旧结果推断；未来字段出现后会自动纳入。
...
- `outputs/gnn_steps_ablation/similarity_eval/latest/metrics_summary.json`：完整指标。
- `outputs/gnn_steps_ablation/similarity_eval/latest/group_summary.csv`：三组汇总表。
- `outputs/gnn_steps_ablation/similarity_eval/latest/metric_long.csv`：长表指标，便于后续制图或论文表格。
- `outputs/gnn_steps_ablation/similarity_eval/latest/sample_features.csv`：逐样本特征。