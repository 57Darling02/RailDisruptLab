...
本项目的主要入口是可视化界面和 `/api`。用户流程不再提供独立 workflow CLI；耗时任务统一由 backend 写入快照后交给 Pueue 执行。

核心原则：

- `core` 只做原子算法能力：原计划解析、MILP 建模、求解、扰动图转换和 VAE 数学图转换。
- `backend` 负责数据流、任务快照、队列、日志、场景表单构造、批量采样、模型训练和生成流程编排。
- `frontend` 负责交互、状态展示和图形化分析。
- 实验参数由 Web 表单或 API 显式提交，并由 backend 写入任务快照，保证可追踪、可复现。
...
`projects/*` 是运行沙箱，不作为源码维护对象。首次使用时在 Web 右上角新建 project，
并通过页面表单完成原计划激活、场景构造和后续任务参数配置。
...
runner 读取快照后进入 backend workflow，再调用 core 原子函数。任务日志由 Pueue 管理，前端通过任务面板查看命令、状态和日志。

这种方式让参数、产物和日志天然绑定到同一个 task，也便于在前端任务面板中回溯实验行为。
...
项目 ID 由 Web 表单输入，项目目录由 backend 创建。

2. 激活原计划运行图

前端上传原始时刻表、里程表并填写 sheet 名称，backend 保存输入文件并调用 core 生成：
...
`core` 不管理项目目录、不管理任务队列、不服务 HTTP、不设计 UI 状态、不构造实验场景、不编排实验流程，只提供可组合的算法能力。

`backend` 不实现 MILP/模型算法，只负责参数快照、任务调度、数据读取、场景构造、时刻表导出、模型训练/生成流程编排和 API。
